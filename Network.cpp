...
void Network::wilsonCycleNetwork(void)
{
	...
	double output_array[networkDimension-1]={0,0,0,0};
    double weight_matrix[networkDimension-1][networkDimension-1]; //2d matrix for weights

    //create initial output array which takes 0,0,1,0 from the text file as it's values to start
    for( neuron_number = 1; neuron_number < networkDimension; ++neuron_number){
        output_array[neuron_number-1]=neuronActivation[neuron_number];
    }
    //copy matrix weight values from neural network into 4x4, for simple matrix multiplication
    for( neuron_number = 1; neuron_number < networkDimension; ++neuron_number){
		for( source_neuron_number = 1; source_neuron_number < networkDimension; ++source_neuron_number){
...
for(timestep; timestep < endtimestep ; timestep++){
    ...
    for(int i=0; i<networkDimension-1; i++){
        for(int k =0; k<networkDimension-1; k++){
        neuron_outputs[i]+= (weight_matrix[k][i])*(output_array[k]);
        }}

        //store neuron outputs in output_array for next loop
    for(int i=0; i<networkDimension-1; i++){
